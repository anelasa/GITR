...
def create_particle_source(nP=1e3,filename='../input/profiles.nc',geom_file='gitrD3DGeometry2DWringsPy.cfg'):
    ...
    nP = int(nP)
    x = 1.4096 + np.random.rand(1,int(nP))*(1.454-1.4096)
    y = np.zeros(int(nP))
    z = -1.25*np.ones(int(nP))+1.0e-5 #0.1 for charged part, no ioniz
    vx = np.zeros(nP)
    vy = -3.0e3*np.ones(nP)
    vz = 3.0e3*np.ones(nP)
    #x = []
...
if __name__ == "__main__":
    create_particle_source(nP=1e3,filename='../input/profiles.nc',geom_file='input/gitrD3DGeometry2DWringsPy.cfg')
